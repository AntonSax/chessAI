...
class AI(BaseAI):
    ...
    def end(self, won, reason):
        ...
        # Print how much time remaining this AI had at the end of the game.
        print("Time Remaining: " + str(self.player.time_remaining) + " ns")
        # 853038296839 ns for minimax
        # 782101854447 ns for a-b_minimax
    ...
    def run_turn(self):
        ...
        result_of_minimax = self.time_limited_iterative_deepening_alpha_beta_depth_limited_minimax()
    ...
    def king_move_space(self, Board, from_file_index, from_rank_index, engaged_space=False):
        ...
        list_of_possible_spaces = []
        list_of_current_players_pieces = []
        ...
        for row in Board:
            ...
            for piece in row:
                ...
                # Check if the piece is on the same team as the king to castle.
                if (piece.istitle() == Board[king_rank_index][king_file_index].istitle()):
                    # Check if the piece is a rook.
                    if piece == "R" or piece == "r":
                        # Define the queenside and kingside rook's rank and file.
                        continue
    ...
    def generate_board_state_score(self, Board, player):
        ...
        decimal_rank_index = 0.0

        # Generate the score for a board state.
        for row in Board:
            decimal_rank_index = decimal_rank_index + 0.1
            white_pawn_advance = (decimal_rank_index - 0.1)
            black_pawn_advance = (0.6 - decimal_rank_index)
            for piece in row:
                if piece == "P":
                    board_state_score_white = board_state_score_white + 1 + white_pawn_advance
                if piece == "p":
                    board_state_score_black = board_state_score_black + 1 + black_pawn_advance
                if piece == "R":
    ...
    # This function checks to see if the board state is a draw, but only because of threefold repetition.
    # The parameter is the board state to evaluate.
    # Returns a boolean value. True if there is a draw. False is there is not.
    ...
    # This function checks to see if the board state is a draw, but only because of the fifty move rule.
    # The parameter is the board state to evaluate.
    # Returns a boolean value. True if there is a draw. False is there is not.
    def check_board_state_for_fifty_move_rule(self, Board):
        #print("\nRunning check_board_state_for_fifty_move_rule()")
        boolean_draw = None

        if self.game.turns_to_draw < 45:
            boolean_draw = True
        else:
            boolean_draw = False

        #print("Exiting check_board_state_for_fifty_move_rule.\n")
        return boolean_draw
    ...
    def check_board_state_for_checkmate(self, Board):
        ...
        # Check to see if the ally has any possible moves.
        if self.possible_move_list(Board, True):
            boolean_checkmate = False
        # If the player doesn't have any possible moves then the player is in checkmate.
        else:
    ...
    # This function runs the minimax algorithm, limited to a certain depth so it doesn't break the computer and additionally does alpha-beta pruning.
    # Should be initially called with alpha and beta at very high values, such as negative infinity and infinity.
    ...
    #               quiescent_depth - the amount of times we may go past a non-quiescent state before stopping.
    #               maximizing_player - a True/False value so we know if we need to maximize or minimize
    # Returns:      A list that is the score of the best move and the board state after the best move is made.
    def history_table_quiescent_search_time_limited_alpha_beta_depth_limited_minimax(self, Board, start_time, time_limit, alpha, beta, depth, quiescent_depth, boolean_quiescent, history_table, maximizing_player):
        ...
        board_state_fifty_move_draw = self.check_board_state_for_fifty_move_rule(Board)
        best_value = None
        best_node = Board
        top_level_child = Board
        child_node_list = []

        # Time control
        elapsed_time = time.time() - start_time

        if quiescent_depth == 0 or (depth == 0 and boolean_quiescent == False) or board_state_checkmate or board_state_insufficient_draw or board_state_threefold_draw:
            ...
            if board_state_threefold_draw:
                if maximizing_player == True:
                    board_state_score = board_state_score + 10
                elif maximizing_player == False:
                    board_state_score = board_state_score - 10
            if board_state_checkmate:
                if maximizing_player == True:
                    board_state_score = board_state_score - 100
                elif maximizing_player == False:
                    board_state_score = board_state_score + 100
            if depth == 0:
        ...
        # Sort the history table based on its 3rd item so values with higher scores come first.
        history_table.sort(key=itemgetter(2), reverse=True)
        # Go through all the possible children (the frontier) and add values to the history table.
        for child in child_node_list:
            # If there is no history for that parent-child pair, add it to the history table.
            history_node = [Board, child, 0]
            create_new_node = False
            for node in history_table:
                if (node[0] == history_node[0]) and (node[1] == history_node[1]):
                    create_new_node = False
                    break
                elif (node[0] == history_node[0]) and (node[1] != history_node[1]):
                    create_new_node = True
            if create_new_node:
                history_table.append(history_node)
        ...
        if maximizing_player == True:
            best_value = -1000
            # This should start with the highest values in the history table.
            for node in history_table:
                print("node in history_table")
                self.print_my_board(node[1])
                print(node[2])
                # Only grab values relevant to this state of the game.
                if node[0] == Board and node[1] != Board:
                    child = node[1]
                # Check to see if a king was in check or a piece was captured and say the state is non-quiescent.
                boolean_non_quiescent_state = self.check_for_non_quiescence(Board, child, maximizing_player)
                if depth == 1 and boolean_non_quiescent_state:
                    print("--------------------------------------------Non Quiescent------------------------")
                    new_list = self.history_table_quiescent_search_time_limited_alpha_beta_depth_limited_minimax(child, start_time, time_limit, alpha, beta, depth, quiescent_depth-1, True, history_table, False)
                else:
                    new_list = self.history_table_quiescent_search_time_limited_alpha_beta_depth_limited_minimax(child, start_time, time_limit, alpha, beta, depth-1, quiescent_depth, False, history_table, False)
                ...
                # New code for alpha-beta
                best_value = max(best_value, new_value)
                alpha = max(alpha, best_value)
                if beta <= alpha:
                    # This is the beta cut-off point
                    break
                # End new code for alpha-beta

                # Stop looking for new nodes if the time limit is exceeded
            ...
            # Update the best node to the history table.
            for node in history_table:
                if (node[0] == history_node[0]) and (node[1] == history_node[1]):
                    history_node[2] = history_node[2] + 1
        ...
        # Maximizing player == False, its the minimizing player's turn.
            ...
            for node in history_table:
                ...
                if depth == 1 and boolean_non_quiescent_state:
                    print("--------------------------------------------Non Quiescent------------------------")
                    new_list = self.history_table_quiescent_search_time_limited_alpha_beta_depth_limited_minimax(child, start_time, time_limit, alpha, beta, depth, quiescent_depth-1, True, history_table, True)
                else:
                    new_list = self.history_table_quiescent_search_time_limited_alpha_beta_depth_limited_minimax(child, start_time, time_limit, alpha, beta, depth-1, quiescent_depth, False, history_table, True)
                new_value = new_list[0]
                ...
                if beta <= alpha:
                    # This is the alpha cut-off point
                    break
                # End new code for alpha-beta
    ...
    # This function when called keeps track of how much time has passed for the iterative_deepening_alpha_beta_depth_limited_minimax. It doesn't run minimax pas a certain time limit.
    # Parameters:
    # Returns:      A list that is the score of the best move and the board state after the best move is made.
    def time_limited_iterative_deepening_alpha_beta_depth_limited_minimax(self):
        ...
        starting_alpha = -1000
        starting_beta = 1000
        history_table = [[self.Board, self.Board, 0]]
        # 1000000000 ns = 1 second
        # Time limit in seconds
        time_limit = 20
        # Add one to depth limit to include it within the range.
        for depth in range(1, depth_limit_int+1):
            print("\n\nDepth:", depth)
            start_time = time.time()
            best_move = self.history_table_quiescent_search_time_limited_alpha_beta_depth_limited_minimax(self.Board, start_time, time_limit, starting_alpha, starting_beta, depth, quiescent_depth, False, history_table, True)
    ...
    # This function checks to see if the king is in check. It is for determining which states are non-quiescent.
    # Parameters:   Board - The board made of a list of lists.
    # Returns:      A boolean value of whether the king is in check or not.
    def is_king_in_check(self, Board, player):
        boolean_king_in_check = False
        king_letter = None
        enemy_possible_move_list = None
        file_index = None
        rank_index = None
        file = None
        rank = None

        # Designate which king is the ally king.
        if (self.player.color == "White" and player == True) or (self.player.color == "Black" and player == False):
            king_letter = "K"
        elif (self.player.color == "Black" and player == True) or (self.player.color == "White" and player == False):
            king_letter = "k"

        # Find the rank and file of the king on the ally side.
        rank_counter = 0
        for row in Board:
            ...
            for space in row:
                ...
                if space == king_letter:
                    rank_index = rank_counter
                    file_index = file_counter
                    break

        
        # Change the index values into real values.
        if rank_index != None and file_index != None:
            king_space_list = self.index_to_board_space_list(rank_index, file_index)

        enemy_possible_move_list = self.possible_move_list(self, Board)

        # Run through the move list, checking to see if the third and fourth item are identical to the kings file and rank.
        if enemy_possible_move_list:
            for move in enemy_possible_move_list:
                if move[2] == king_space_list[0]:
                    if move[3] == king_space_list[1]:
                        boolean_king_in_check = True
                        break
                    else:
                        boolean_king_in_check = False
                else:
            ...
            #print("The enemy's move list was empty.")
            pass

        return boolean_king_in_check

    # This function checks to see if at the given Board state, was a piece removed from the board.
    # Intended to be inserted in the Minimax function after a child node has been generated and selected.
    # Parameters:   Board - The board made of a list of lists.
    #               child - A child node of the current Board.
    # Returns:      A boolean value of whether or not a piece was captured.
    def is_a_piece_captured(self, Board, child):
        boolean_piece_captured = None
        board_piece_count = 0
        child_piece_count = 0

        for row in Board:
            for space in row:
                # Check to see if the space is anything but empty.
                if space != ".":
                    board_piece_count = board_piece_count + 1

        for row in child:
            for space in row:
                if space != ".":
                    child_piece_count = child_piece_count + 1

        print("Parent Piece Count:", board_piece_count, "Child Piece Count:", child_piece_count)

        if board_piece_count != child_piece_count:
            boolean_piece_captured = True
        else:
            boolean_piece_captured = False

        return boolean_piece_captured

    # A function to check if a pawn is promoted.
    #def is_a_pawn_promoted(self, Board):

    # A function to check if the state of the Board can be declared non-quiescent.
    # Parameters:   Board - The board made of a list of lists.
    #               child - A child node of the current Board.
    #               player_color - The color of the Board's player.
    # Returns:      A boolean value of whether or not the current Board is non-quiescent.
    def check_for_non_quiescence(self, Board, child, player):
        #print("Running check for non-quiescence.")

        boolean_non_quiescent_state = None
        boolean_king_in_check = None
        boolean_is_piece_captured = None

        boolean_king_in_check = self.is_king_in_check(Board, player)
        boolean_is_piece_captured = self.is_a_piece_captured(Board, child)

        if boolean_king_in_check or boolean_is_piece_captured:
            boolean_non_quiescent_state = True
            print("boolean_non_quiescent_state: True")
        else:
            boolean_non_quiescent_state = False

        #print("Exiting check_for_non_quiescence.")
        return boolean_non_quiescent_state
